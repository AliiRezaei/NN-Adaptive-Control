...
% plant :
problem.plant.fx = @(x) [x(2); (1-x(1))^2 * x(2) - x(1)]; % f(x)
...
problem.auxiliary.K = diag([-1, -1]);
problem.auxiliary.h = -10 * ones(n, 1);
...
problem.desParam.Ac    = diag([-.05, -.05]);
...
zeta_0  = [0; 0];        % auxiliary states init cond
V_hat_0 = V;             % hidden layer weights init cond
W_hat_0 = W;             % output weights init cond

InitCond = [x_0; zeta_0; V_hat_0(:); W_hat_0(:)]; % initial condition
odeFunc  = @(t, x) NNACIS(t, x, problem);         % ode function
[~, X]   = ode45(odeFunc, t, InitCond);           % solve ode
nStates  = size(X, 2);                    % number of states
nSteps   = size(X, 1);                    % number of states
...
% input layer weights evolution :
c = 1;
V_hat = zeros(hiddenLayerSize, inputLayerSize, nSteps);
...
% output layer weights evolution :
W_hat = zeros(outputLayerSize, hiddenLayerSize, nSteps);
...
for i = 1:hiddenLayerSize
    for j = 1:inputLayerSize
        plot(t, squeeze(V_hat(i, j, :)), 'LineWidth', 1)
        hold on
    end
end
...
for i = 1:outputLayerSize
    for j = 1:hiddenLayerSize
        plot(t, squeeze(W_hat(i, j, :)), 'LineWidth', 1)
        hold on
    end
end
xlabel('$t$ $[sec]$', 'FontSize', 15)
